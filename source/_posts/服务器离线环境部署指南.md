...
```bash
# 将本地的虚拟环境打包（直接解压至miniconda的envs目录下就可以使用）
...
安装前需要确保服务器拥有gcc，g++，make，binutils软件包（使用apt下载），gcc需要手动软链接cc工具
...
1. 在清华镜像源下载所需版本的系统资源（[Ubuntu22.04](https://mirrors.tuna.tsinghua.edu.cn/ubuntu-releases/)），选择`desktop-amd64.iso`结尾的文件下载
...
5. 将U盘插入主机，开机过程中一直按F12，直至进入BIOS系统，选择使用U盘启动
